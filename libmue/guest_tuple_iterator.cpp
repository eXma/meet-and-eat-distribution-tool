...
mue::Guest_tuple_generator::Guest_tuple_generator(std::vector<Team*> const& guests, std::unordered_set<Team_id> const& used)
:
	_possible_guests(guests),
	_guest_end(
		std::remove_if(_possible_guests.begin(), _possible_guests.end(),
			[used](const Team* t) -> bool { return used.find(*t) != used.end(); }))
{}
